...
const SAMPLE = [
  ...
  'Despesa,"R$ 869,59",2026-09-23,CG 160 Fan (5/48),-,Conta - Banco Inter,Veículo,Falta pagar',
  'Despesa,"R$ 1.130,00",2026-08-06,Pedro ,-,Conta - Banco Inter,Empréstimo,Já foi pago',
  'Receita,"R$ 280,20",2026-08-16,Ifood ,-,Conta - Banco Inter,Rendimento,Já recebido',
  'Despesa,"R$ 192,49",2026-09-30,Serasa ,Claro,Conta - Banco Inter,Pagamento,Falta pagar',
].join('\n')
...
describe('leitura de célula', () => {
  ...
  it('separa a parcela do nome', () => {
    expect(parseInstallment('CG 160 Fan (5/48)')).toEqual({
      description: 'CG 160 Fan',
      installment: { n: 5, total: 48 },
    })
    expect(parseInstallment('Econ Construtora (2/24)').installment).toEqual({ n: 2, total: 24 })
  ...
  it('tira o prefixo do rótulo da conta', () => {
    expect(cleanAccountLabel('Conta - Banco Inter')).toBe('Banco Inter')
    expect(cleanAccountLabel('Cartão - Nubank')).toBe('Nubank')
    expect(cleanAccountLabel('Banco Inter')).toBe('Banco Inter')
  })
})
...
describe('linhas prontas', () => {
  ...
  it('converte o extrato inteiro', () => {
    const [moto, pedro, ifood, serasa] = rows()
    ...
    expect(pedro).toMatchObject({ amountCents: 113000, paid: true, description: 'Pedro' })
    expect(ifood).toMatchObject({ kind: 'income', amountCents: 28020, paid: true })
    // "-" é célula vazia; "Claro" é complemento de verdade.
    expect(serasa!.detail).toBe('Claro')
  })
  ...
  it('lista os rótulos distintos para o mapeamento', () => {
    ...
    expect(distinctLabels(rows(), (row) => row.accountLabel)).toEqual(['Banco Inter'])
  })
})
